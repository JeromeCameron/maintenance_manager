...
@router.get("/supplier/{supplier_id}", status_code=status.HTTP_200_OK, response_model=list[PurchaseOrder])
async def get_purchase_orders_by_supplier(supplier_id: int, session: Session = Depends(get_session)):
    return purchase_orders.get_purchase_orders_by_supplier(session, supplier_id)
