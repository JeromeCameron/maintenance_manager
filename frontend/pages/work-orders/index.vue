...
const statusStyles: Record<string, string> = {
  requested:      "bg-blue-50 text-blue-600 ring-1 ring-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:ring-blue-500/30",
  scheduled:      "bg-indigo-50 text-indigo-600 ring-1 ring-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:ring-indigo-500/30",
  awaiting_parts: "bg-amber-50 text-amber-700 ring-1 ring-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:ring-amber-500/30",
  awaiting_po:    "bg-orange-50 text-orange-600 ring-1 ring-orange-200 dark:bg-orange-500/10 dark:text-orange-400 dark:ring-orange-500/30",
  in_progress:    "bg-yellow-50 text-yellow-700 ring-1 ring-yellow-200 dark:bg-yellow-500/10 dark:text-yellow-400 dark:ring-yellow-500/30",
  on_hold:        "bg-slate-100 text-slate-500 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-700",
  cancelled:      "bg-red-50 text-red-500 ring-1 ring-red-200 dark:bg-red-500/10 dark:text-red-400 dark:ring-red-500/30",
  ...
  closed:         "bg-emerald-50 text-emerald-600 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:ring-emerald-500/30",
}
...
const typeStyles: Record<string, string> = {
  ...
  inspection:   "bg-sky-50 text-sky-600 dark:bg-sky-500/10 dark:text-sky-400",
  project:      "bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400",
}
...
<template>
  <div class="flex min-h-full flex-col gap-4">
          ...
          <div class="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div class="flex shrink-0 items-start gap-4 border-b border-gray-100 dark:border-slate-800 px-6 py-5">
              ...
              <div class="flex-1 min-w-0">
                <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">New Work Order</h3>
                <p class="mt-0.5 text-sm text-gray-500 dark:text-slate-400">Fill in the details to create a new work order</p>
              </div>
    ...
    <UCard :ui="{ root: 'flex flex-col flex-1 min-h-0', body: 'flex flex-col flex-1 min-h-0 p-0' }">
      ...
      <!-- Tabs -->
      <UTabs v-model="activeTab" :items="tabs" :ui="{ root: 'flex flex-col flex-1 min-h-0 w-full', list: 'border-b border-gray-100 dark:border-slate-800 px-5 pt-1 rounded-none bg-white dark:bg-slate-900 shrink-0', content: 'flex-1 min-h-0' }">
        <template #todo>
          <div class="overflow-auto h-full p-4 space-y-2">
            <div v-if="filteredTodo.length === 0" class="py-12 text-center text-sm text-gray-400 dark:text-slate-500">
            ...
            <template v-else>
              ...
              >
                ...
                <div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-slate-800">
                  <UIcon name="i-heroicons-clipboard-document-list" class="h-4 w-4 text-gray-400 dark:text-slate-500" />
                </div>
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div class="flex items-center gap-1.5">
                    <UIcon v-if="wo.asset_pm_id" name="i-heroicons-arrow-path" class="h-3.5 w-3.5 shrink-0 text-gray-400 dark:text-slate-500" />
                  ...
                  <!-- Asset -->
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-slate-400">Asset: {{ wo.asset_id ?? '—' }}</p>
                  ...
                  <div class="mt-1.5 flex flex-wrap items-center gap-2">
                    <span
                      ...
                      :class="typeStyles[wo.typ] ?? 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400'"
                    >{{ wo.typ }}</span>
                    <span v-if="wo.supplier_id && supplierMap[wo.supplier_id]" class="flex items-center gap-1 text-[11px] text-gray-500 dark:text-slate-400">
                      <UIcon name="i-heroicons-building-storefront" class="h-3 w-3" />
                      {{ supplierMap[wo.supplier_id] }}
                    </span>
                    <span v-if="wo.issue_date" class="flex items-center gap-1 text-[11px] text-gray-400 dark:text-slate-500">
                  ...
                  <div class="mt-2 flex items-center gap-2">
                    ...
                    >
                      <span
                        ...
                        :class="statusStyles[wo.status] ?? 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400'"
                      >
                ...
                <div class="shrink-0 flex flex-col items-end gap-1">
                  ...
                  <span class="text-xs text-gray-500 dark:text-slate-400">#{{ wo.work_order_id }}</span>
                  <div class="flex flex-col items-end gap-0.5 mt-0.5">
                    ...
                    <span class="flex items-center gap-1.5 text-[11px] text-gray-600 dark:text-slate-300 font-medium">
                      <span
        ...
        <template #done>
          <div class="overflow-auto h-full p-4 space-y-2">
            <div v-if="filteredDone.length === 0" class="py-12 text-center text-sm text-gray-400 dark:text-slate-500">
            ...
            <template v-else>
              <div
                ...
                :key="wo.work_order_id"
                class="flex cursor-pointer items-start gap-4 rounded-lg border-l-4 border-l-transparent px-5 py-4 ring-1 ring-gray-200 dark:ring-slate-700 hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-colors"
                @click="openEdit(wo)"
              >
                <!-- Left icon -->
                <div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-slate-800">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-sm font-semibold text-slate-500 dark:text-slate-400 truncate">
                      {{ wo.description ? (wo.description.length > 60 ? wo.description.slice(0, 60) + '…' : wo.description) : `Work Order #${wo.work_order_id}` }}
                    </span>
                  </div>
                  <!-- Asset -->
                  <p class="mt-0.5 text-xs text-gray-400 dark:text-slate-500">Asset: {{ wo.asset_id ?? '—' }}</p>
                  <!-- Meta row -->
                  <div class="mt-1.5 flex flex-wrap items-center gap-2">
                    <span
                      v-if="wo.typ"
                      ...
                      :class="typeStyles[wo.typ] ?? 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400'"
                    >{{ wo.typ }}</span>
                    <span v-if="wo.supplier_id && supplierMap[wo.supplier_id]" class="flex items-center gap-1 text-[11px] text-gray-400 dark:text-slate-500">
                      <UIcon name="i-heroicons-building-storefront" class="h-3 w-3" />
                      {{ supplierMap[wo.supplier_id] }}
                    </span>
                    <span v-if="wo.date_completed" class="flex items-center gap-1 text-[11px] text-gray-400 dark:text-slate-500">
                      <UIcon name="i-heroicons-check" class="h-3 w-3" />
                      Completed {{ formatDate(wo.date_completed) }}
                    </span>
                    <span v-else-if="wo.issue_date" class="flex items-center gap-1 text-[11px] text-gray-400 dark:text-slate-500">
                      <UIcon name="i-heroicons-calendar" class="h-3 w-3" />
                      Issued {{ formatDate(wo.issue_date) }}
                    </span>
                  </div>
                ...
                <div class="shrink-0 flex flex-col items-end gap-1">
                  <div class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-200 dark:bg-slate-700">
                    <UIcon name="i-heroicons-user-group" class="h-4 w-4 text-slate-400 dark:text-slate-500" />
                  </div>
                  <span class="text-xs text-gray-400 dark:text-slate-500">#{{ wo.work_order_id }}</span>
                  <span class="flex items-center gap-1.5 text-[11px] text-gray-400 dark:text-slate-500 font-medium mt-0.5">
                    <span
