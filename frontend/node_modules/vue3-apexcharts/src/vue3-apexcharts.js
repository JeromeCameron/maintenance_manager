...
const vueApexcharts = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // Deep clone that preserves function values (e.g. axis label formatters),
    // which JSON.stringify would otherwise drop, causing ApexCharts to fall
    // back to its default (over-precise) number formatting after an update.
    const copyData = (data) => {
      ...
      for (const key in data) {
        if (Object.prototype.hasOwnProperty.call(data, key)) {
          out[key] = typeof data[key] === "function" ? data[key] : copyData(data[key]);
        }
      }
      return out;
    };
